...
def cotangent_weights(verts, faces): return igl.cotmatrix(verts, faces)

def mass_matrix(verts, faces):
  n_verts = len(verts)
  M = np.zeros((n_verts, n_verts), dtype=np.float64)
  for face in faces:
    v0, v1, v2 = verts[face]
...
def laplace_beltrami_operator(verts, faces, cotangent_weights):
  n_verts = len(verts)
...
def dynamic_time_warping(seq1, seq2):
  ...
  return dtw_matrix[-1, -1], normalized_dtw_distance.item()
...
def face_coords(verts, faces): return verts[faces]
def cross(vec_A, vec_B): return torch.cross(vec_A, vec_B, dim=-1)
def project_to_tangent(vecs, unit_normals): return vecs - unit_normals * dot(vecs, unit_normals).unsqueeze(-1)
def normalize(x, divide_eps=1e-6, highdim=False): return x / (norm(x) + divide_eps).unsqueeze(-1)
...
def compute_operators(verts, faces, k_eig, normals=None):
  ...
  faces_np = toNP(faces).astype(int)
